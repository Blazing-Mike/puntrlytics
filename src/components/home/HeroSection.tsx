...
export function HeroSection({ activeProviderUrl }: { activeProviderUrl: string }) {
  return (
    <div className="mx-auto max-w-5xl text-center">
      <Badge variant="outline" className="mb-4 inline-flex items-center gap-2 rounded-full border-rule bg-ticket2/40 px-3 py-1 font-mono text-[11px] font-bold uppercase tracking-widest text-gold hover:bg-ticket2/40 border">
      ...
      <div className="mt-8 flex flex-col sm:flex-row items-center justify-center gap-6">
        <BookmarkletLink
          ...
          className="group inline-flex items-center justify-center gap-2.5 rounded-lg bg-lime px-8 py-3.5 text-[17px] font-bold leading-none text-blacktop no-underline shadow-[0_16px_38px_rgba(65,212,132,.25)] transition hover:-translate-y-0.5 hover:bg-[#54dd8e] hover:shadow-[0_22px_44px_rgba(65,212,132,.35)]  focus-visible:outline-3 focus-visible:outline-offset-3 focus-visible:outline-cyan motion-reduce:transition-none max-[520px]:w-full"
        >
          Analyze my bets

        </BookmarkletLink>

        <Button render={<Link href="/demo" />} variant="link" className="font-utility font-bold uppercase tracking-wider text-sm text-gold transition hover:text-cyan hover:no-underline px-0">
          View sample report
        </Button>
      </div>
