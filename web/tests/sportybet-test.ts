...
const raw = JSON.parse(
  fs.readFileSync(path.join(root, "sportybet-sample.json"), "utf8"),
);

const bets = raw.data.entityList.map((o: unknown) =>
  normalizeOrder(o as never),
);
...
// parseDate now renders in LOCAL time — compute the expected date the same way.
{
  const t0 = new Date(1780845952081);
  const pad = (n: number) => String(n).padStart(2, "0");
  const expectedDate =
    t0.getFullYear() + "-" + pad(t0.getMonth() + 1) + "-" + pad(t0.getDate());
  check("bet1 date", bets[0].date.slice(0, 10), expectedDate);
}
...
for (const b of bets)
  console.log(
    `  ${b.betId} | ${b.date} | stake ${b.stake} | payout ${b.payout} | odds ${round(b.odds)} | ${b.status}`,
  );
